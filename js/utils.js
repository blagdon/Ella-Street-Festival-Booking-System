...
// ===================================================================
// === EPIC 4A: Organisation/Event slug validation ===
// ===================================================================
// Mirrored (not imported — different runtimes) in
// supabase/functions/_shared/slugs.ts for the Edge Function side. Keep the
// two RESERVED_SLUGS lists and the format rule in sync by hand; there's no
// shared module system between browser ESM and Deno here.
//
// Covers every real top-level page basename (a slug matching one would be
// confusing even though it can't literally collide with a same-segment
// static file — the /{orgSlug}/{eventSlug} resolver only ever fires for
// exactly two path segments) plus generic infrastructure words and the
// Phase 1 sentinel ids (org_default/event_default), which must stay
// reserved so a real organisation can never claim them.
 ...
 * message safe to show directly to an admin; returns the cleaned slug on
 * success. Deliberately rejects invalid input rather than silently
 * stripping characters — two different intended slugs silently stripping
 * down to the same string is exactly the kind of surprise this exists to
 * prevent.
 * @param {string} value
 * @param {string} [label] used in error messages, e.g. "Organisation slug"
 */
