...
Multi-Event Architecture Phase 1. An organisation having more than one event was already possible in the data model (`bookings.event_id`/`locations.event_id`, added Phase 2 of the tenant-awareness work) but not yet safe in practice — two concrete defects, both confirmed live in production (`org_default` already has two events: `event_default`/ESF26 open, a second draft event/ESF28), are closed here.

### Fixed

- **MISC booking IDs were generated from organisation-level settings, not the specific event being booked into.** `rpc_get_next_misc_id()` read `settings.booking_prefix` rather than the event's own `events.booking_prefix`, so a second event with its own distinct prefix would still generate MISC ids under the first event's/org-level prefix. Now takes `p_event_id`, verifies the event genuinely belongs to the caller's own organisation, and reads that event's own prefix — bringing this in line with `submit-booking`'s existing per-event prefix resolution for ordinary bookings. The approved table-level locking strategy (`LOCK TABLE bookings IN SHARE ROW EXCLUSIVE MODE`, unchanged from the original `get_next_booking_id()`/`rpc_get_next_misc_id()` pattern) was reviewed separately and left exactly as-is — deliberately out of scope here, along with a related pre-existing insert-side race in the MISC booking flow, both logged as known follow-ups.
- **`events.booking_prefix` had no uniqueness constraint at all.** Both `get_next_booking_id()` and `rpc_get_next_misc_id()`'s own max-id scans match against the *entire* `bookings` table with no org/event filter, so correctness has always implicitly depended on prefix uniqueness holding globally — this was never actually enforced. Now a real `UNIQUE` constraint, deliberately global (not per-organisation, since the scan itself isn't scoped) and deliberately not relaxed for closed/archived events (their old bookings remain in the same globally-scanned table). Verified against production before deployment: the four prefixes in use (`AF27`, `ESF26`, `ESF28`, `S1SM`) were already distinct, so the constraint applied cleanly with zero conflicts.
- **Admin-side queries mixed bookings/locations from every event of an organisation together.** `fetchKanbanData`, `fetchPayments`, and `fetchLocationData` (both its display and occupancy-check queries) now filter by `event_id`; `insertMiscBooking` now writes `org_id`/`event_id` explicitly rather than relying on column defaults; a steward's own sync query (`page-steward.js`) is event-scoped the same way. `page-admin.js`'s create/edit-event error handling now distinguishes the new `booking_prefix` collision from the pre-existing slug collision, previously reported as one generic message.
- **A steward whose organisation's real event wasn't `event_default` synced zero bookings**, caught by CI immediately after the above landed. Resolving a steward's organisation (`resolveStewardOrgId()`) clears the cached active event but nothing repopulated it — stewards have no event-switcher UI to do so themselves — so the new event-scoped query above fell back to a hardcoded `event_default` that didn't match their org's real event. Fixed by resolving the steward's own active event the same way the admin event-selector already does (`fetchAvailableEvents()`, scoped to their organisation), immediately after their organisation resolves.
...
- **Three orphaned Edge Functions removed from production.** `visitor-map`, `public-map`, and `dynamic-action` had no source anywhere in this repository and predated every other function. Two were publicly callable with no auth and queried `bookings`/`locations` directly with the anon key — harmless only as an accident of this app's own RLS grants having been revoked elsewhere, not by their own design. The disaster-recovery runbook's own documented `--no-privileges` restore gap would have silently reopened exactly the grants these depended on. Deleted; production and repository Edge Function inventories now match exactly (17/17), confirmed live.
- **Newly-provisioned organisations were silently missing more than half their email/SMS templates.** `platform_defaults_email_templates` had 4 of the 9 templates the app actually uses; `platform_defaults_sms_templates` had 3 of 6, one of them (`cancellation_confirmed`) using an id `cancel-booking` never actually queries. Every organisation provisioned via the wizard hit this on close to its first booking — live-reproduced: confirming a free/community stall failed to email the trader. Both tables now carry the full set, content adapted from `org_default`'s own working copies; the SMS id mismatch is corrected to `booking_cancelled`. Verified on a freshly-provisioned organisation: 9/9 email and 6/6 SMS templates cloned, and the full booking lifecycle (confirm chargeable, confirm free, reject, assign location, HCC batch check, payment reminder) now fails only on the test project's own deliberate absence of Zoho credentials — never on a missing template.
- **A related cross-tenant data-integrity bug, exposed by the fix above.** Three admin-side queries (Email Admin, SMS Admin, HCC Dashboard's batch-send) fetched or updated `email_templates`/`sms_templates` by id alone, with no organisation filter — harmless while at most one organisation ever had a given template id, but now that every organisation does, a platform-admin "View As" session editing one organisation's template would have silently overwritten every organisation's copy at once, and the HCC batch-send would have started failing outright the moment two organisations shared an `hcc_batch_check` row. Fixed to filter by the current organisation, matching the pattern already used everywhere else in the codebase. The same missing-filter pattern, also found in two permanent test files (one of which was failing outright on the full-suite run because of it), is fixed too.
- **Edge Function deployment automated.** A new CI job deploys every function to production on push to `main`, gated behind every other check passing — closing the one manual step left in an otherwise fully automated release pipeline. Live-verified on the merge that shipped it: all 17 functions redeployed with fresh versions, the four public ones (`submit-booking`, `cancel-booking`, `stripe-webhook`, `get-payment-link`) correctly kept `verify_jwt: false`.
- **The disposable test project's own cleanup was silently broken.** `scripts/seed-test-project.mjs` has always tried to delete every non-`org_default` organisation at the start of each run, but never cleaned up `organisation_members` first — a real foreign key — so the final delete failed with an uncaught error every single time, for any organisation that had ever had a member added. ~55 organisations had accumulated as a result. Fixed the cleanup order and made every step check its own error; the accumulated mess (organisations plus a handful of now-orphaned bookings/locations/audit log rows) is cleaned up. Confirmed live: the test project now holds exactly one organisation.
- **Two stale documentation claims corrected.** `USER_GUIDE.md`'s login instructions pointed at an abandoned Vercel URL instead of the real production domain. `HANDOVER.md`'s "Known gaps" section claimed no error/alerting integration existed; Sentry has in fact been live on both the browser and Edge Function sides for some time — the doc was simply never updated.
- **A real accessibility gap on `hcc_dashboard.html` fixed.** The per-row council-status, approval-date, and select-booking controls (desktop and mobile) had no accessible name at all. Found because a leftover manual test fixture happened to be the first populated row the accessibility suite had ever scanned — the underlying gap was pre-existing, just never previously exercised.

### Investigated, not changed

- The `documents` storage bucket, flagged as "unused" by the initial audit pass, was investigated before any action was taken: it holds 16 real files belonging to the separate performers admin application (the same ownership pattern already documented for the `performers`/`schedules` tables), not dead infrastructure. The original finding was corrected rather than acted on — deleting it would have destroyed another application's real data.
...
Epic 1–4 multi-tenant rollout. Epic 4 (Phases 4A–4D) finishes the public-facing
multi-tenant surface: organisations and events get real public identity, physical
locations get a proper admin module, event-level configuration overrides land, and
every public booking path resolves its organisation/event from the URL instead of
assuming `org_default`. The certification rounds that followed are the larger part
of this release by volume: a genuinely new organisation, walked end-to-end by three
personas (organiser, trader, platform administrator) with no SQL or manual DB
intervention allowed, repeatedly found that a table's RLS "worked" only because
`org_default` was the only real tenant — the moment a second organisation existed,
several tables let one organisation's admin read or act on another's data. Every
one of those findings, across four separate rounds of fixes, is closed in this
release.

### Added

- **Public Identity & Event Resolution (Phase 4A).** `validateSlug()`/`RESERVED_SLUGS` (`js/utils.js`, mirrored in `supabase/functions/_shared/slugs.ts`) enforce lowercase-hyphenated organisation/event slugs. New anon-readable views `public_organisations_info` (id/slug/name/website only, no `contact_email`) and `public_events_info` (excludes `draft` events, so a visitor can't resolve a still-being-configured event by guessing its slug). A new `/{orgSlug}/{eventSlug}` pretty URL resolves through `js/public-context.js` to a minimal `event.html` landing page showing the organisation's branding and the event's status.
- **Location Management (Phase 4C).** Full admin module (`js/page-admin-locations.js`) replacing manual-SQL pitch management with Add/Edit/Delete/Duplicate/Import-CSV/Export-CSV/Clone (same-event or cross-organisation)/bulk-delete. `locations`' composite `(id, dataset)` primary key has no `org_id`, so two organisations picking the same generic pitch code collide directly rather than being scoped away — Add/Edit surface that as a plain "that code's taken" message; Duplicate/Import/Clone auto-suffix instead, since those bulk-populate ids the admin didn't hand-choose.
- **Event-scoped configuration overrides (Phase 4B + 4B.1).** New `event_settings` table adds the missing Event layer to the Platform → Organisation → Event inheritance chain. New Event Configuration page (`event_settings.html`) lets an organiser override Festival Display Name, the three stall prices, and Allowed Stall Types for just the active event; "Reset to Organisation Default" deletes the override row rather than copying the current value down, so a reset event keeps tracking the organisation default automatically. Booking Prefix deliberately excluded — it already has its own resolution path via `events.booking_prefix`.
- **Public booking forms resolve their organisation/event from the URL (Phase 4D).** `General_Booking.html`/`Food_Stall_booking.html` read `?org=&event=` slugs and gate the form on that event's status; `submit-booking` resolves and persists the real organisation/event server-side, never from client-supplied `org_id`/`event_id`. Its event-lifecycle guard (reject unless the resolved event's `status` is `'open'`) is now unconditional. `cancel-booking`, `stripe-webhook`, and `create-checkout-session` send confirmation/cancellation/payment-request emails and texts using the booking's own organisation.
- **Self-service organisation onboarding.** Adding an owner (`provision-organisation`) or a team member (`rpc_add_organisation_member` / new `invite-organisation-member` Edge Function) now sends a genuine Supabase invite email via `inviteUserByEmail()` instead of only ever creating a placeholder `user_roles` row nobody was told about. `js/page-index.js`'s password-recovery landing now also handles `type=invite` links with welcoming copy.
- **`rpc_get_organisation`, `rpc_get_public_locations`, and `has_org_role()`.** Explicit, labelled SECURITY DEFINER bypasses for the platform-admin "View As" and anon-public-map read paths, replacing implicit blanket RLS bypasses that had no correlation to which organisation was actually being requested.
- **Deno local type-checking.** `supabase/functions/deno.json` makes `deno check` work locally against every Edge Function — the tool that caught 13 real compile errors in `provision-organisation` below.
- **Combined `npm test` script** running setup, integration, and both accessibility suites in sequence.

### Fixed

Three rounds of live, no-SQL operational certification against genuinely fresh
organisations, plus a fourth pass sweeping the codebase for every other instance of
each pattern found — not just the tables named by a given round's own findings.

- **No organisation could ever get an independent admin login.** Both provisioning paths only ever created a placeholder `user_roles` row with a random UUID — no invite email, no self-service signup page. Fixed as part of the onboarding work above.
- **Every System Settings save silently corrupted `org_default` instead of the selected organisation.** Six settings files omitted `org_id` from their `.upsert()` calls; `settings.org_id` defaults to `'org_default'`, so every save from any other organisation landed there regardless of the org switcher, with a false "success" toast.
- **A `try`/`catch` scoping bug in `provision-organisation` silently broke rollback and swallowed the real error** on any mid-provisioning failure — `let`/`const` declared inside `try` but referenced in the sibling `catch`, so the rollback itself threw a fresh `ReferenceError` instead of running.
- **`event_settings.event_id` had no foreign key to `events.id`**, letting a write against a nonexistent event succeed silently and leave orphaned rows behind.
- **"NONFOOD" contains "FOOD" as a substring.** `submit-booking`'s `booking_type` computation and the HCC-checks gate both miscategorised every General/Non-Food booking as food via a plain `.includes('FOOD')`. Extracted one `isFoodPrefix()` helper per runtime, used at all three sites.
- **The org switcher and its RLS policies leaked every organisation's existence to any admin, and let any org-scoped admin write to any *other* organisation's row.** Three `organisations` policies all granted a blanket admin bypass; split into per-command policies with a new `is_platform_admin()` helper that checks only genuine platform-admin (`org_default`) membership, not "admin of any single org."
- **The Payments Dashboard's "Pending" total silently excluded `Payment Requested` bookings**, showing £0.00 for a real awaiting-payment booking.
- **Two parallel Settings UIs, one of them writing keys nothing reads** — the newer Settings Hub wrote `food_applications_open`/`general_trader_applications_open`/`sentry_loader_url`, names nothing else in the app recognised. Toggling "open"/"closed" saved successfully and had zero effect on the public forms.
- **Booking form pages never displayed the resolved organisation/event's identity** — the header stayed hardcoded to "Ella Street Festival 2026" regardless of which organisation's link a trader followed.
- **Organisation Settings could silently show and let an admin overwrite a *different* organisation's identity.** The RLS tightening above removed the blanket admin `SELECT` bypass this tab relied on without migrating it to the replacement RPC pattern; also stopped substituting fabricated placeholder data on a failed/denied fetch, since that fabrication is what turned a read-side gap into a write-side risk.
- **Team & Members showed the platform-wide staff roster with no organisation filter at all**, and Change Role/Remove wrote the global `user_roles` table — revoking a member's access everywhere, not just the organisation being viewed.
- **New organisations launched showing the literal seed placeholder "Festival Event"** instead of the name entered in the setup wizard, on every screen driven by `festival_display_name`.
- **`bookings`, `locations`, `events`, `booking_locations`, `email_templates`, and `sms_templates` RLS had no tenant correlation.** All six gated on a role check with a documented global fallback: any organisation's owner already satisfied it for every *other* organisation too. Realigned with `is_platform_admin()` as the explicit bypass and `has_org_role()` otherwise.
- **Anonymous visitors could read every organisation's live pitch layout in one request** — the public map's location policy filtered only by `dataset='LIVE'`, no organisation filter at all. Anon's direct table read is now revoked entirely; reads go through `rpc_get_public_locations(org_id, dataset)`.
- **Every organisation's Stripe checkout, payment-link, and refund activity ran against `org_default`'s credentials**, regardless of which organisation the customer actually booked with — `loadStripeSettings()`'s `orgId` argument defaulted silently and every call site omitted it.
- **Bulk-messaging admin gates and the client-side template fetch weren't organisation-scoped.** `queue-bulk-email`/`queue-bulk-sms`/`retry-queued-email`/`retry-queued-sms` checked only the legacy global role table; `js/message-templates.js` had no `org_id` filter despite templates using a composite `(org_id, id)` key.
- **The root cause underneath the above: every organisation's owner was silently granted `org_default` (platform-admin) membership** as a side effect of an unrelated provisioning trigger writing to the legacy global role table — confirmed live in both the disposable test project and production. The trigger now only ever updates an *existing* `org_default` row, never creates one; `is_platform_admin()` checks real `org_default` membership instead of the polluted global table.
- **`payments`, `audit_logs`, `hcc_checks`, `email_queue`, and `sms_queue` had the identical unscoped-admin RLS gap**, found by sweeping for every other instance of the pattern above rather than stopping at the tables a given round's findings happened to name. A related, independent bug: `org_id` on all five defaults to `'org_default'`, and nearly every write site across the codebase (the Stripe auto-confirm path, every transactional booking email/SMS, the HCC-checks insert) omitted it, silently mistagging every row regardless of the real organisation — fixed at every write site, not just the RLS.
- **`organisation_members` itself — the access-control table — had the same unscoped gap**, proven live as one organisation's admin promoting a *different* organisation's own steward to admin. `event_settings` had an equivalent gap with no organisation filter at all. `performers`/`schedules` were checked and correctly left alone: a genuinely separate, single-tenant feature with no organisation concept, not an unswept instance of this pattern.
- **A shared modal focus-trap/Escape implementation, independently reimplemented in `kanban.js` and `summary.js`** under different names — exactly why `summary.js`'s modals were missed when Escape-key handling was first added to every modal in the app. Consolidated into `trapModal()`/`releaseModal()` in `js/ui.js`.
- **The Gmail invite-link `otp_expired` behaviour, investigated and classified as an integration risk** (most likely link-scanner prefetch consuming the one-time token), not an application defect — reproduced directly by navigating the exact URL rather than assumed.

### Testing

- `tests/phase4a-public-identity.test.mjs`, `tests/phase4c-location-management.test.mjs` — slug validation/reservation, the anon-safe public views, cross-organisation pitch-code collisions.
- `tests/phase4b-event-configuration.test.mjs`, `e2e/event-configuration.spec.mjs` — inheritance, override creation/removal/reset, `event_settings` RLS.
- `tests/phase4d-public-booking-routing.test.mjs`, `e2e/public-booking-routing.spec.mjs` — slug resolution, the lifecycle guard, client-supplied `org_id`/`event_id` being ignored, the legacy no-slug fallback.
- `tests/tenant-isolation.test.mjs` (new) — every RLS/write-path finding above proven real against the disposable test project first (a genuine cross-tenant reproduction, not a policy-text read), then confirmed closed after the fix, across all four rounds.
- `tests/security.test.mjs`, `tests/privilege-hardening.test.mjs`, `tests/user-roles-enum.test.mjs` updated for the anon-locations mechanism change and the RLS policies' new (still behaviourally equivalent) implementations.
- `e2e/focus-trap.spec.mjs`, full admin accessibility suite — the shared modal refactor.
- Full regression suite (356 integration tests, 35 admin accessibility/e2e tests) clean on both the test project and production before each production deploy in this release.
