...
import { service, anonKey, url } from '../tests/helpers.mjs';
...
/**
 * Confirms a just-created auth user is actually ready for password sign-in
 * before any test drives the real login UI against it. Exists because CI
 * reproduced (twice, identically) a UI-login timeout immediately after
 * createUser() had already returned success with no error - never
 * reproduced locally. GoTrue occasionally isn't queryable for sign-in the
 * instant after a service-role createUser() call resolves; this closes
 * that window with a direct API probe instead of guessing a fixed delay.
 *
 * Bounded and narrow: only retries GoTrue's own 'invalid_credentials'
 * response, which is the *only* failure this can reach - the password used
 * here is the one generated by this file and handed to createUser()
 * moments earlier, never typed or transcribed, so a genuine credential
 * mismatch isn't a reachable outcome. Any other error (network failure, a
 * different AuthApiError code, etc.) is a real problem and is rethrown on
 * the first attempt rather than retried.
 * @param {string} email
 * @param {string} password
 */
async function waitForStewardSignInReady(email, password) {
  const attempts = 4;
  const delayMs = 1500;
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    if (attempt === attempts) {
      throw new Error(`Steward account ${email} still not sign-in-ready after ${attempts} attempts: ${error.message}`);
    }
    console.log(`[steward readiness] ${email}: sign-in not ready yet (attempt ${attempt}/${attempts}, ${error.code}) - retrying in ${delayMs}ms`);
...
async function createSteward(key) {
  ...
  await waitForStewardSignInReady(s.email, s.password);
  return s.userId;
}
