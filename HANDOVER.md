...
> version of the same bug, because the export is what gets reconciled against
> the bank. It now carries Refund Amount / Refunded On / **Net Paid**. The
> export was also silently ignoring three of the six payment filters, because
...
| `create-checkout-session` | Admin JWT only | Creates a Stripe Checkout Session for any not-yet-resolved booking (called directly from the chargeable-confirm modal, or again on "Resend Payment Request" from `Payment Requested`), saves the resolved `stall_cost`, updates the booking to `Payment Requested` (also storing the full Checkout URL in `stripe_checkout_url`, for `get-payment-link` below), and emails the `payment_requested` template with the session URL. With `send_sms: true`, also sends the `payment_requested` SMS — carrying a short `pay.html?token=<stripe session id>` redirect link rather than the raw ~400+ character Stripe URL. Picks the `stripe_secret_key_test`/`_live` settings-table row by whether `instance_prefix` contains `-DEV-` (or the `stripe_test_mode` override — see below). See [Stripe Payment Collection](#stripe-payment-collection) below. |
| `get-payment-link` | None (`--no-verify-jwt`) | Public, read-only lookup backing `pay.html`: resolves `bookings.payment_link_code` (an 8-char url-safe random code every booking gets by default, `20260731110000_payment_link_short_code.sql`) — the token in the short SMS payment link — back to the full `stripe_checkout_url` stored on the booking, refusing anything not currently `Payment Requested`. No Turnstile — unlike `cancel-booking` this has no side effect to gate. |
...
supabase functions deploy get-payment-link --no-verify-jwt
```
...
71. **Ran the last untested link — `refund-payment` itself, the admin-initiated
    path (2026-07-22, agent-driven via the localhost test-project override, no
    code changed).** Deliberately not automatable per the function's own header
    — *"Admin JWT only, with no service-role bypass: issuing a refund moves real
    money and must always be a deliberate human action"* — and item 64 separately
    records that an automated suite able to move money in any mode isn't worth
    having. Signed into the Payments page as the test admin (`esfUseTestProject`
    override, banner confirmed), clicked **Refund**, filled notes, confirmed both
    the amount-entry modal and the *"This cannot be undone from here"*
    double-confirmation, and the button issued a real Stripe **test-mode** refund.
    Verified independently at every layer, not just the on-screen toast: Stripe's
    own charge record (`refunded: true, amount_refunded: 4500/4500`), the
    `payments` row (`refund_reference: "re_3TvtJJJnN335NAbh0ONDG9p6"` — a real
    refund id, correctly *not* a charge id, which is exactly what item 69's
    webhook fix was about), and `audit_logs` (`refund_issued_stripe`, attributed
    to the actual signed-in test admin — the tell that distinguishes this
    admin-button path from the webhook's `'Stripe (automatic)'` attribution).
    **The Stripe API call, the RPC, the audit trail, and the UI refresh
    (badge flips to REFUNDED, Paid/Refunded totals move by the right amount, the
    Refund button disappears) all worked correctly on the first real attempt.**

    **One real, minor finding, not fixed here — deliberately.** The pre-existing
    `E2EREFUND-A` fixture (left over from item 69, still `PAID`, unrefunded)
    could **not** be refunded through the button: `js/api.js`'s
    `refundStripePayment()` calls `validateBookingId()` **client-side, before**
    ever invoking the Edge Function, and `E2EREFUND-A` doesn't match the
    `ESF26-{TYPE}-\d{4}` pattern — it was hand-inserted with a memorable custom
    id rather than generated by `get_next_booking_id`. The Edge Function itself
    has no such restriction (`refund-payment/index.ts` just does
    `.eq('id', booking_id).single()`), so this can **never** block a real
    admin refunding a real booking — every real id is server-generated and
    always matches. Not worth "fixing": loosening the client check would only
    remove a fast, correct early error for a case that cannot occur on real
    data. Worked around by creating a **fresh** fixture instead
    (`ESF26-DEV-0001`) — id allocated via the actual `get_next_booking_id` RPC
    (never hand-pick a DEV-prefix id: the function computes `MAX(existing)+1`
    live off the table, not a separate sequence, so an arbitrary high number
    would permanently break the 4-digit assumption for every future DEV
