...
| `get-payment-link` | None (`--no-verify-jwt`) | Public, read-only lookup backing `pay.html`: resolves a Stripe Checkout Session id (the token in the short SMS payment link) back to the full `stripe_checkout_url` stored on the booking, refusing anything not currently `Payment Requested`. No Turnstile — unlike `cancel-booking` this has no side effect to gate. |
